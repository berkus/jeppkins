...
class ContactGroup(skypekit.Object):
    ...
    def CanAddConversation(
        ...
        B{Return values:}
         - B{result} - true if AddConversation(contact) works on this group
        """
  ...
  """
    ...
    def CanAddContact(
        ...
        B{Return values:}
         - B{result} - true if AddContact(contact) works on this group
        """
...
class ContactSearch(skypekit.Object):
  ...
  - P_ABOUT 
    ...
    def AddStrTerm(
        ...
        B{Arguments:}
         - B{prop} - only selected contact properties
         - B{cond}
         - B{value}
         - B{add_to_subs}
...
class Participant(skypekit.Object):
  ...
  930, True), self.transport)
    ...
    def _sk_get_adder(self):
        """
        [ALL] who added this participant
        """
...
class Conversation(skypekit.Object):
  ...
  """
    ...
    def OnMessage(
        ...
        """
        This callback gets fired when a message gets added or changed in the Conversation. 
        
        
        B{Return values:}
         - B{message} - Message for which the OnMessage event got fired. 
...
class Message(skypekit.Object):
  ...
  - propagates the changes to remote users. 
        ...
        Specify isXML as true if the message body is formatted as XML; omit it or specify it as false if the message body is plain text. 
...
class Video(skypekit.Object):
  ...
  - Participant::VIDEO_STATUS = STREAMING 
    ...
    def _sk_get_dimensions(self):
        """
        'width height', for example '640 480'
        """
    ...
    def SetScreen(
        ...
        """
        Assigns window handle to the video object, where the rendering should occur. 
    ...
    def Start(self):
        """
        This method starts either video send or video receive, depending on whether the video object is sender or receiver. If the video object is supposed to start rendering video locally, Video class Start method must be preceeded by calls to SetScreen and SetRenderRectangle 
...
class Account(skypekit.Object):
  ...
  - PWD_OK_BUT_CHANGE_SUGGESTED - consider displaying an updated indicator, along with a recommendation to change again to a stronger password. 
    ...
    def _sk_get_logoutreason(self):
        """
        Recognized values for the P_LOGOUTREASON property. 
...
class Skype(object):
  ...
  :'IS_WEBCAM',
    ...
    def GetAvailableOutputDevices(self):
        """
        This method returns a table in form of three string lists of equal length. The first list contains audio output device handles ('hw:0,0', 'hw:0,1', etc.) The second list contains descriptive names of those devices (Ensoniq AudioPCI etc.) The third list contains device product IDs. Note that the values in these lists depend on which audio engine you are running (SAL, PCM, RTP). 
  ...
  """
    ...
    def App2AppRead(
        ...
        NB! There is currently no easy way to determine how many bytes were actually read from the stream. To get around this limitation, you can prefix all stream packets with a header that contains the packet payload size. 
